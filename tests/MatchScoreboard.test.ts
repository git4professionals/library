import { MatchScoreboard } from "../src/MatchScoreboard";
...
describe("MatchScoreboard", () => {
  let scoreboard: MatchScoreboard;
  ...
  test("should handle matches with the same total score ordered by start time from the requiremnts", async () => {
//a. Mexico 0 - Canada 5
//b. Spain 10 - Brazil 2
...
// Introduce a delay of 1 second before starting the next match
await delay(1000);
...
await delay(1000);
    ...
    expect(summaryMatches[1].homeTeam).toBe("Spain");
    expect(summaryMatches[1].awayTeam).toBe("Brazil");
    expect(summaryMatches[1].homeScore).toBe(10);
    expect(summaryMatches[1].awayScore).toBe(2);

    expect(summaryMatches[2].homeTeam).toBe("Mexico");
    expect(summaryMatches[2].awayTeam).toBe("Canada");
    expect(summaryMatches[2].homeScore).toBe(0);
    expect(summaryMatches[2].awayScore).toBe(5);

    expect(summaryMatches[3].homeTeam).toBe("Argentina");
    expect(summaryMatches[3].awayTeam).toBe("Australia");
    expect(summaryMatches[3].homeScore).toBe(3);
    expect(summaryMatches[3].awayScore).toBe(1);

    expect(summaryMatches[4].homeTeam).toBe("Germany");
    expect(summaryMatches[4].awayTeam).toBe("France");
    expect(summaryMatches[4].homeScore).toBe(2);
    expect(summaryMatches[4].awayScore).toBe(2);
